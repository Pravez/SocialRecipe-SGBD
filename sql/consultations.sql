...
SELECT id_menu, menu_name, id_user FROM menu;
...
SELECT id_menu, NbRecipeValid.menu_name
...
--recettes présentes dans au moins trois menus, ayant au moins 10 notes et au moins 3 commentaires

(SELECT recipe.id_recipe FROM recipe
JOIN is_part_of ON recipe.id_recipe = is_part_of.id_recipe
GROUP BY recipe.id_recipe
HAVING COUNT(id_menu) >=3 --au moins trois menus
INTERSECT
SELECT recipe.id_recipe FROM recipe
JOIN note ON recipe.id_recipe = note.id_recipe
GROUP BY recipe.id_recipe
HAVING COUNT(id_user) >=10) --au moins 10 notes
INTERSECT
SELECT recipe.id_recipe FROM recipe
JOIN comment ON recipe.id_recipe = comment.id_recipe
GROUP BY recipe.id_recipe
HAVING COUNT(id_user) >=3; --au moins 3 commentaires


--Consultation détaillée du menu @menu
SELECT id_menu, menu_name, id_user
FROM menu
WHERE id_menu = @menu;
SELECT recipe.id_recipe, recipe_name
FROM recipe
JOIN is_part_of ON recipe.id_recipe = is_part_of.id_recipe
WHERE id_menu = @menu;
